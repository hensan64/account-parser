...
class Converter {
    ...
    static void execute(Bank bank, String inPath, String outPath, String prefix) throws IOException {
    ...
    private static void write(List<Data> dataList, String filePath) throws IOException {
        ...
        for (Data data : dataList) {
            if (data.type == "inflow")
                ...
                throw (new Error("Wrong value type: Neither 'inflow' | 'outflow': " + data.type.toString()));
            // Payee (not used)
            lines.add(data.year + "-" + data.month + "-" + data.day + "," + "," + data.prefix + "," + data.memo + ","
                    + valueString);
        }
